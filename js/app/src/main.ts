...
if (env.name === "development") {
  appStreamlitServer = spawn("poetry", ["run", "enopios", "gui", "--electron"]);
} else {
  appStreamlitServer = spawn("./enopios",  ["gui", "--electron"], {
...
app.on("ready", () => {
  ...
  streamlitPortDelegate.promise.then((port) => {
    const testwebappAppUrl = url.format({
      pathname: `localhost:${port}`,
      protocol: "http:",
      slashes: true,
    });

    mainWindow.loadURL(testwebappAppUrl);
  });
});
